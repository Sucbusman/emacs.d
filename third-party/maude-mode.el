...
;;         Santiago Saavedra <s.saavedra@ucm.es>
;; Maintainer: Santiago Saavedra <s.saavedra@ucm.es>
;; Keywords: Maude
;; Version: 0.1.0
;; Homepage: https://github.com/ssaavedra/maude-mode
...
;; This is a GNU Emacs mode for Maude. It is tested to work
;; successfully in Emacs 24. I modified this from original work from
;; Ellef Gjelstd, whose maude-mode worked for XEmacs previously.

;; Things that work:
;;  - `RUN-MAUDE' for running an inferior maude process
;;  - Syntax highlighting of basic maude

;; I don't know about the rest because I am still figuring my way in Maude.
...
(defcustom maude-command "maude"
  "The path where your Maude executable resides. You will
  probably need to set this variable in order for `RUN-MAUDE' to work."
  :group 'maude)


;;;###autoload
(defgroup maude nil
  "Major mode for editing Maude programs."
  :link '(custom-manual "(maude-mode)")
  :group 'languages
  :prefix "maude-")
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defalias 'maude-parent-mode
  (if (fboundp 'prog-mode) 'prog-mode 'fundamental-mode))

;;;###autoload
(define-derived-mode maude-mode maude-parent-mode "Maude"
...
;; Add the mode to the auto-mode-alist for .maude extension
;;;###autoload
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Works on Linux
;; This is derived from comint-mode
(require 'comint)
(require 'derived)
...
(defun copy-current-buffer-to-maude ()
  (interactive)
  (save-buffer)  
  (comint-send-string maude-buffer (concat "in " (buffer-file-name) "\n")))


(define-derived-mode maude-running-mode
  comint-mode "Maude running"
  "Major mode for running Maude. This is derived from
`comint-mode'."
  (font-lock-mode)
...
;; 									"sort" "sorts" "op" "ops" "var" "vars" "mb" "mbs" "eq" "eqs" "rl" "rls"
									...
									"pr" "inc" "is" "class" "cmb" "rl" "crl" "msg" "msgs" "ceq" "strat" "sd"
;; 									"ctor" "id"
									"and" "or" "else" "fi"
...
(defun maude-in-comment-p ()
  "Return t if this is in a comment.  Currently handles only monoline comments"
  (interactive)
  (save-excursion
...
;; 		("identity" "[id:"  (lambda () (maude-mode-join-attributes) (maude-mode-place-after "id:"))  0)
		...
		("endsm" "endsm" (lambda () (save-excursion (indent-line-to 0))))
		...
		("smod" "smod" (lambda () (save-excursion (indent-line-to 0))))
